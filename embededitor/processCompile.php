...
// function score()
// {
// 	$sql="SELECT * from userdata where uid=13 AND qid='$pbid'";
...
// 			$sql="UPDATE users set done='$done',points='$points' where uid=13";
// 			$result=mysqli_query($conn,$sql);
// 		}
// 	}else {
// 		$sql="INSERT INTO userdata(uid,qid,solved) VALUES('13','$pbid','1')";
// 		$result=mysqli_query($conn,$sql);
// 	}
// }
$totalPoints=0;
...
if ($done==1) { // done=1 means user has all test case correct
    ...
    if (mysqli_num_rows($result)>0) {
        ...
        if ($data['solved']==0) {
            ...
            $sql="UPDATE users set done='$done',pbSol=pbSol+1,points='$totalPoints' where uid=".$_SESSION['uid']."";
            $result=mysqli_query($conn, $sql);
        }else {
          $totalPoints=$userData['points'];
        }
    } else {
        $sql="INSERT INTO userdata(uid,qid,solved) VALUES('".$_SESSION['uid']."','$pbid','1')";
        $result=mysqli_query($conn, $sql);
        $points=$codeData['diff'];
        $totalPoints=$userData['points']+$points;
        $sql="UPDATE users set done='$done',pbSol=pbSol+1,points='$totalPoints' where uid=".$_SESSION['uid']."";
